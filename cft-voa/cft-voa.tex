...
\begin{document}
    ...
    % Should be d/dt (sum exp phi) rather than sum (d/dt exp phi), somehow get only phi_n and phi_-n, probably get \delta_n+m,0 somehow (n, m indices of two sums)?
    % ????????????
    % Actually, I think I'm missing cross terms such as d/dt d/dx ... which come from sum?
    \begin{align}
