...
\begin{document}
    ...
    A derivation is just any linear map satisfying the Leibniz rule, and the \enquote{adjoint representation} is simply \(\lie{g}\) acting on itself where \(X\) acts on \(Y\) by \(X \cdot Y \mapsto \bracket{X}{Y}\). % TODO 3
    ...
    \begin{remark}{}{}
        ...
        The \(\nilpotentLie\) stands for \enquote{nilpotent} because \(\nilpotentLie(n, \field)\) is a (maximal) nilpotent algebra.
