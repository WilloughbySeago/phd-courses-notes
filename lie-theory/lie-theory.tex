...
\begin{document}
    ...
    The \enquote{adjoint representation} is simply \(\lie{g}\) acting on itself where \(X\) acts on \(Y\) by \(X \cdot Y \mapsto \bracket{X}{Y}\). % TODO 3
    ...
    An important observation here is that if we define the map\footnote{This is the so called adjoint representation.} \(\ad_H \colon \specialLinearLie(2, \complex) \to \specialLinearLie(2, \complex)\) by \(\ad_H(X) = \commutator{H}{X}\) then we see that this map has \(E\) and \(F\) as eigenvectors, with eigenvalues \(2\) and \(-2\) respectively. % TODO 2
    ...
    This requires replacing the indexing set \(\complex \setminus \{0\}\) with the root space \(\lie{h}^* \setminus \{0\}\), or rather we've already done that, it's just that \(\dim \lie{h} = 1\) so \(\lie{h}^* \isomorphic \lie{h}\) and \(\dim \lie{h}^* = 1\) so \(\lie{h}^* \isomorphic \complex\). % TODO 4
    ...
    \begin{remark}{}{}
        Here \(\borelLie\) stands for \enquote{Borel} because \(\borelLie(n, \field)\) is a \defineindex{Borel subalgebra}, meaning it's a maximal solvable subalgebra (we'll define solvable shortly). % TODO 1
        The \(\nilpotentLie\) stands for \enquote{nilpotent} because \(\nilpotentLie(n, \field)\) is a (maximal) nilpotent algebra (we'll define nilpotent shortly). % TODO 6
...
%        \end{proof}
    ...
    \begin{dfn}{Nilpotent Lie Algebra}{}
        ...
        A Lie algebra is \defineindex{nilpotent} if its lower central series eventually terminates with \(0\).
    ...
    \begin{lma}{}{}
        ...
        Then there exists a maximal (with respect to inclusion) solvable ideal of \(\lie{g}\) called the \defineindex{radical} of \(\lie{g}\), denoted \(\rad \lie{g}\).
        \begin{proof}
    ...
    % TODO: add reference to definition of solvable Lie alg in remark (TODO 1)
    % TODO: add reference to definition of adjoint representation to discussion of sl(2, C) (TODO 2)
    % TODO: add reference to definition of adjoint representation to discussion of Jacobi identity as derivation (TODO 3)
    % TODO: add reference to definition of (semi)simple Lie alg to discussion of sl(2, C) (TODO 4)
    % TODO: add reference to definition of representation to discussion of sl(2, C) (TODO 5)
    % TODO: add reference to definition of nilpotent Lie alg in remark (TODO 6)
