...
\begin{document}
    ...
    Note that \(\Hom_A(M, N)\) has the structure of an \(A\)-module with operations defined pointwise.
    When \(M\) is an \((A, B)\)-bimodule, \(N\) is a left \(B\)-module, and \(P\) is a left \(B\)-module there is an isomorhpism
    \begin{equation}
        \Hom_B(M, \Hom_B(N, P)) \xrightarrow{\isomorphic} \Hom_B(M \otimes_A N, P)
    \end{equation}
    This is given by
    \begin{equation}
